                                     ...
                                     mtr_t *mtr) noexcept
{
  ...
  else if (block->page.lock.have_u_not_x())
  {
    ...
    /* Our U latch prevented invalidate(); compare the raw values,
    because modify_clock_matches() rejects a saturated one. */
    ut_ad(block->page.modify_clock() == modify_clock);
  }
...
void buf_page_t::log_modify_clock_saturated(const char *message) noexcept
{
