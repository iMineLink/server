...
bool buf_page_t::set_accessed() noexcept
{
  ...
  if (access_time)
    return true;
  const uint16_t now= buf_pool_t::now();
  /* access_time == 0 is the "never accessed / just made young" sentinel, so a
  real access must never stamp 0. This is now ? now : 1 computed without a
  branch: (uint32_t{now} - 1) >> 31 is 1 only when now == 0, so it lifts just
  that value to 1 and leaves every other stamp exact. */
...
{
  return uint16_t(my_interval_timer() / 1000000000ULL);
}

void buf_pool_t::old_threshold_and_now(uint32_t &threshold, uint16_t &now)
  const noexcept
{
  threshold= LRU_old_time_threshold;
  now= threshold ? buf_pool_t::now() : 0;
}
