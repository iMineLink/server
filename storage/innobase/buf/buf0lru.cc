...
@param limit      maximum number of blocks to scan
@param now        uint16_t(my_interval_timer() / 1e9), or 0 if threshold is 0
@param threshold  buf_pool.LRU_old_time_threshold, read once by the caller
                                             ...
                                             uint32_t threshold)
{
	...
	for (buf_block_t* block = UT_LIST_GET_LAST(buf_pool.unzip_LRU);
		...
		if (block->page.zip.was_accessed()) {
			block->page.make_young(now, threshold);
		} else {
...
static bool buf_LRU_scan_and_free_block(ulint limit)
{
  ...
  return buf_LRU_free_from_unzip_LRU_list(limit, now, threshold) ||
    buf_LRU_free_from_common_LRU_list(limit, now, threshold);
}
