...
private:
  ...
  /** Saturation value of modify_clock(); see modify_clock_values_match() */
  static constexpr uint64_t MODIFY_CLOCK_MAX= (uint64_t{1} << 48) - 1;

#if defined(UNIV_DEBUG)
  static void log_modify_clock_saturated(const char *message) noexcept;
#endif
...
public:
  ...
  void init(uint32_t state, page_id_t id, uint16_t ssize) noexcept
  {
    ...
    match. */
    DBUG_EXECUTE_IF("ib_modify_clock_saturate_low",
                    modify_clock_low= uint32_t(MODIFY_CLOCK_MAX);
                    modify_clock_high= uint16_t(MODIFY_CLOCK_MAX >> 32););
    access_time= 0;
...
public:
  ...
  /** @return number of invalidate() calls, saturated at MODIFY_CLOCK_MAX */
  ...
  optimistic restore. Reject a stored MODIFY_CLOCK_MAX: invalidate() does
  not advance a saturated counter, so two readings after saturation
  compare equal even if invalidate() was called between them.
  ...
  static bool modify_clock_values_match(uint64_t current, uint64_t stored)
    noexcept
  {
    ut_ad(stored <= MODIFY_CLOCK_MAX);
    if (UNIV_UNLIKELY(stored == MODIFY_CLOCK_MAX))
    {
      DBUG_EXECUTE_IF("ib_modify_clock_saturate_log",
                       log_modify_clock_saturated("InnoDB: modify_clock_values_"
                                                   "match rejecting saturated"
                                                   " value");
                      );
      return false;
    }
  ...
  @param count_not_young  whether to count an old block that was
                          accessed but not flagged, in
                          buf_pool.stat.n_pages_not_made_young
  @return whether the block was moved */
  bool make_young_if_needed(bool count_not_young= false) noexcept;
};
...
inline void buf_page_t::invalidate() noexcept
{
  ...
  /* Do not advance a saturated counter; see modify_clock_values_match().
  At about 1 M invalidate()/s on one descriptor, saturation takes about
  9 years. */
  if (UNIV_UNLIKELY(modify_clock_low == uint32_t(MODIFY_CLOCK_MAX) &&
                     modify_clock_high == uint16_t(MODIFY_CLOCK_MAX >> 32)))
  {
    DBUG_EXECUTE_IF("ib_modify_clock_saturate_log",
                     log_modify_clock_saturated("InnoDB: invalidate() reached"
                                                 " modify_clock max");
                    );
    return;
  }
