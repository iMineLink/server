...
private:
  ...
  Split out from page_cleaner_status because this bit has a cross-thread
  access pattern (mutated by buf_flush_wait(), buf_flush_ahead(), and
  page_cleaner_wakeup() from non-cleaner threads, and read lock-free
  from buf_flush_ahead()), while FLUSH_LIST_ACTIVE / LRU_FLUSH in
  page_cleaner_status are cleaner-internal.  Placed in the alignment
  hole after page_cleaner_status; cache-line neighbours are all
  flush_list_mutex-protected, so the lock-free read only false-shares
  with correlated cleaner-state writes. */
