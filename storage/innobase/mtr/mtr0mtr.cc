...
{
  buf_flush_ahead(flush_lsn & ~lsn_t{1}, bool(flush_lsn & 1));
}
...
void mtr_t::commit_log(mtr_t *mtr, std::pair<lsn_t,lsn_t> lsns) noexcept
{
  ...
  {
    ut_ad(lsns.second <= (mtr->m_commit_lsn | 1));
...
static lsn_t log_close(lsn_t lsn) noexcept
{
  ...
  const lsn_t checkpoint_age= lsn - checkpoint;
  ...
  if (UNIV_LIKELY(checkpoint_age < log_sys.max_checkpoint_age))
    return (checkpoint + log_sys.max_modified_age_async) & ~lsn_t{1};

  log_sys.set_check_for_checkpoint();
  return (checkpoint + log_sys.max_checkpoint_age) | 1;
}
