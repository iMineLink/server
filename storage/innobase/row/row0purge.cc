  ...
  DBUG_EXECUTE_IF("enable_row_purge_remove_clust_if_poss_low_sync_point",
	...
	if (mode == BTR_MODIFY_LEAF) {
		...
		success = DB_FAIL != btr_cur_optimistic_delete(
			btr_pcur_get_btr_cur(&node->pcur),
			btr_cur_defer_purge_merges ? BTR_PURGE_DELETE_FLAG : 0,
			&mtr);
		if (success)
...
found:
		...
		if (row_purge_poss_sec(node, index, entry, &mtr)) {
			...
			if (btr_cur_optimistic_delete(
				    &pcur.btr_cur,
				    (btr_cur_defer_purge_merges && index->is_btree())
